...
fn test_safe_token_integration() {
    use atipicial_contract::contract::aep17::{Aep17Token, update_aep17_balance, update_aep17_total_supply};
    
    // Mock token implementation for testing
    struct TestToken;
    
    impl Aep17Token for TestToken {
    ...
    let success = update_aep17_balance::<1>(account, amount);
    assert!(success);
    
    // Test safe total supply update
    update_aep17_total_supply::<0>(amount);
