...
//! 
//! Tests for all Atipicial storage operations including StorageContext,
//! StorageItem, StorageMap, and advanced storage patterns.

#![cfg(test)]

use atipicial_contract::prelude::*;
use atipicial_contract::storage::{StorageContext, StorageItem, StorageMap};
...
mod advanced_storage_patterns {
    ...
    fn test_composite_key_patterns() {
        ...
        let alice_atipicial_key = ByteString::from_literal("balance:alice:ATC");
        let alice_atipicial_balance = Storage::get(ctx.clone(), alice_atipicial_key);
        assert!(alice_atipicial_balance.is_some());
