...
mod native_contracts_tests {
    ...
    #[test]
    fn test_atipicial_contract_operations() {
        // Test ATC token operations
        let account = H160::zero();
        
        // Get ATC balance (mock returns 0)
        let balance = atipicial::get_balance(account);
        assert_eq!(balance, Int256::zero());
        
        // Get ATC token info
        let name = atipicial::get_name();
        assert_eq!(name, ByteString::from_literal("ATC"));
        
        let symbol = atipicial::get_symbol();
        assert_eq!(symbol, ByteString::from_literal("ATC"));
        
        let decimals = atipicial::get_decimals();
        assert_eq!(decimals, 0);
        
        // Test transfer (mock returns true)
        let to = H160([1u8; 20]);
        let amount = Int256::from(100);
        let result = atipicial::transfer(account, to, amount);
        assert!(result);
    }
    ...
    fn test_atipicial_governance_operations() {
        ...
        // Test voting
        let account = H160::zero();
        let voted = AtipicialGovernance::vote(account, Some(pubkey));
        ...
        // Test account state
        let state = AtipicialGovernance::get_account_state(account);
    ...
    fn test_cryptolib_operations() {
        use atipicial_contract::native::cryptolib::{CryptoLib, EcdsaCurve};
        
        let data = ByteString::from_literal("Hello, Atipicial!");
    ...
    fn test_stdlib_extended_operations() {
        use atipicial_contract::native::stdlib_extended::StdLibExtended;
    ...
    fn test_oracle_operations() {
        ...
        use atipicial_contract::contract::native::Oracle;
    ...
    fn test_policy_contract() {
        use atipicial_contract::contract::native::Policy;
    ...
    fn test_role_management() {
        use atipicial_contract::contract::native::RoleManagement;
    ...
    fn test_bls12_381_operations() {
        use atipicial_contract::native::cryptolib::CryptoLib;
