...
impl ByteString {
    ...
    /// Creates a ByteString from a byte slice with proper Atipicial VM integration
    #[inline(always)]
    pub fn from_bytes(bytes: &[u8]) -> Self {
        #[cfg(target_family = "wasm")]
        {
            // For WASM, create ByteString through Atipicial VM syscall for proper type handling
            if bytes.is_empty() {
                Self::empty()
            } else {
                // Create ByteString from bytes using Atipicial VM compatible conversion
                // Convert bytes to hex representation for Atipicial VM compatibility
