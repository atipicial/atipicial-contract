//! ATC Governance Functions
//! Extended governance features for the ATC native contract

use crate::prelude::*;
use crate::types::{ByteString, Int256, H160, PublicKey, Array, Any};

/// ATC contract hash on Atipicial
#[inline(always)]
...
/// ATC Governance operations
pub struct AtipicialGovernance;

impl AtipicialGovernance {
    ...
    pub fn get_account_state(account: H160) -> AccountState {
        ...
        if let Some(arr) = result.as_array::<Any>() {
            // Production implementation: Parse account state from Atipicial contract response
            // Atipicial account state typically contains: [balance, height, vote_to]
            if arr.length() >= 3 {
                ...
                AccountState {
                    ...
                    vote_to: if !vote_to_any.is_null() { 
                        // Convert H160 to PublicKey - in Atipicial, public keys are used for voting
    ...
    /// Calculate bonus GAS for ATC holder
