...
pub trait Aep17Token {
    ...
    fn transfer(from: H160, to: H160, amount: Int256, _data: Array<Any>) -> bool {
        ...
        if amount.is_positive() {
            if !update_aep17_balance::<PREFIX_BALANCE>(from, amount.checked_neg()) {
                return false;
            }
            let _ = update_aep17_balance::<PREFIX_BALANCE>(to, amount);
    ...
    fn mint(account: H160, amount: Int256) {
        ...
        let _ = update_aep17_balance::<PREFIX_BALANCE>(account, amount);
        update_aep17_total_supply::<TOTAL_SUPPLY_KEY>(amount);
    ...
    fn burn(account: H160, amount: Int256) {
        ...
        let _ = update_aep17_balance::<PREFIX_BALANCE>(account, burned);
        update_aep17_total_supply::<TOTAL_SUPPLY_KEY>(burned);
