//! AEP-26: NFT Transfer Callback
//! AEP-27: Token Transfer Callback
...
/// AEP-26: NFT Transfer Callback Interface
...
/// AEP-27: Token Transfer Callback Interface
...
/// AEP-26/27 Implementation Helper
pub struct TransferCallback;

impl TransferCallback {
    /// Invoke AEP-26 callback for NFT transfer
    pub fn invoke_aep26_callback(
        ...
        let result = Contract::call(
            to,
            ByteString::from_literal("onAEP11Payment"),
            CallFlags::ALL,
            Array::from_vec(vec![
                from.into_any(),
                amount.into_any(),
    ...
    /// Invoke AEP-27 callback for token transfer
    pub fn invoke_aep27_callback(
        ...
        let result = Contract::call(
            to,
            ByteString::from_literal("onAEP17Payment"),
            CallFlags::ALL,
            Array::from_vec(vec![
                from.into_any(),
                amount.into_any(),
    ...
    /// Safe transfer with AEP-26 callback for NFTs
    pub fn safe_nft_transfer(
        ...
        if Self::is_contract(to) {
            let callback_result = Self::invoke_aep26_callback(
    ...
    /// Safe transfer with AEP-27 callback for tokens
    pub fn safe_token_transfer(
        ...
        // Invoke callback if recipient is a contract
        if Self::is_contract(to) {
            let callback_result = Self::invoke_aep27_callback(
                H160::zero(), // Would be actual contract hash
                from,
                to,
                amount,
...
/// Helper macro for implementing AEP-26 receiver
#[macro_export]
macro_rules! impl_aep26_receiver {
    ($contract:ty) => {
        impl AEP26Receiver for $contract {
            fn on_aep11_payment(
                &mut self,
                from: H160,
                amount: Int256,
...
/// Helper macro for implementing AEP-27 receiver
#[macro_export]
macro_rules! impl_aep27_receiver {
    ($contract:ty) => {
        impl AEP27Receiver for $contract {
            fn on_aep17_payment(
                &mut self,
                from: H160,
                amount: Int256,
...
impl AEP26Receiver for ReceiverContract {
    fn on_aep11_payment(
        &mut self,
        _from: H160,
        _amount: Int256,
...
impl AEP27Receiver for ReceiverContract {
    fn on_aep17_payment(
        &mut self,
        _from: H160,
        _amount: Int256,
