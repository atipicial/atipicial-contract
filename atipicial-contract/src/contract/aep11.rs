...
// NOTE: atipicial-contract-proc-macros must be updated
...
pub trait Aep11Token<T: TokenState + FromPlaceholder> {
    ...
    fn tokens() -> Iter<T> {
        ...
        {
            // Use Atipicial VM storage iteration to find all tokens with PREFIX_TOKEN
    ...
    fn tokens_of(owner: H160) -> Iter<T> {
        ...
        #[cfg(target_family = "wasm")]
        {
            // Use Atipicial VM storage iteration to find tokens owned by this address
            use crate::services::storage::Storage;
            use crate::types::FindOptions;
            let context = Storage::get_context();
    ...
    fn transfer(to: H160, token_id: ByteString) {
        ...
        // Update balances
        update_aep11_balance(from, token_id.clone(), Int256::minus_one());
        update_aep11_balance(to, token_id.clone(), Int256::one());
    ...
    fn mint(token_id: ByteString, _token_state: T) {
        ...
        // Update balance
        update_aep11_balance(owner, token_id.clone(), Int256::one());
    ...
    fn burn(token_id: ByteString) {
        ...
        // Update balance
        update_aep11_balance(owner, token_id.clone(), Int256::minus_one());
