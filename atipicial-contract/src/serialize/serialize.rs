...
/// Serialize a value to bytes using Atipicial's serialization format
...
impl<T: AtipicialSerializable> Serialize for alloc::vec::Vec<T> {
    fn serialize(&self) -> Result<Bytes, SerializationError> {
        serialize_vec(self)
    }
}

impl<T: AtipicialSerializable> Serialize for Option<T> {
    fn serialize(&self) -> Result<Bytes, SerializationError> {
