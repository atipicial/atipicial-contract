...
use super::atipicial_serializable::{AtipicialSerializable, SerializationError};
...
/// Deserialize a value from bytes using Atipicial's serialization format
...
impl<T: AtipicialSerializable> Deserialize for alloc::vec::Vec<T> {
    fn deserialize(bytes: &[u8]) -> Result<Self, SerializationError> {
        deserialize_vec(bytes)
    }
}

impl<T: AtipicialSerializable> Deserialize for Option<T> {
    fn deserialize(bytes: &[u8]) -> Result<Self, SerializationError> {
