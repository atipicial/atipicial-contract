...
echo "🔧 Fixing Atipicial Contract Examples..."

# Fix serialization import issues
echo "📦 Adding AtipicialSerializable imports..."
find examples -name "*.rs" -exec grep -l "to_bytes\|from_bytes\|deserialize" {} \; | while read file; do
    if ! grep -q "use atipicial_contract::serialize::AtipicialSerializable" "$file"; then
        echo "  Adding AtipicialSerializable import to $file"
        sed -i '/use atipicial_contract::prelude::\*/a use atipicial_contract::serialize::AtipicialSerializable;' "$file"
    fi
done
