...
pub(crate) fn expand_structs_impl(input: proc_macro::TokenStream) -> proc_macro::TokenStream {
    ...
    match input {
        ...
        Ok(_) => {
            return syn::Error::new(Span::call_site(), "`#[atipicial::structs]` can only be applied to `struct`")
                .to_compile_error()
                .into()
        }
    ...
    let mut expanded = match &item.fields {
        syn::Fields::Named(_fields) => {
            quote::quote! {
                ...
                #vis struct #ident #generics {
                    placeholder: atipicial_contract::types::placeholder::Placeholder,
                } #semi_token

                #[cfg(target_family = "wasm")]
                impl #impls atipicial_contract::types::placeholder::FromPlaceholder for #ident #types #wheres {
                    #[inline(always)]
                    fn from_placeholder(placeholder: atipicial_contract::types::placeholder::Placeholder) -> Self {
                        Self { placeholder }
                    }
                }

                #[cfg(target_family = "wasm")]
                impl #impls atipicial_contract::types::placeholder::IntoPlaceholder for #ident #types #wheres {
                    #[inline(always)]
                    fn into_placeholder(self) -> atipicial_contract::types::placeholder::Placeholder {
                        self.placeholder
                    }
                }
            }
        ...
        syn::Fields::Unnamed(_fields) => {
            quote::quote! {
                #[cfg(target_family = "wasm")]
                #(#attrs)*
                #vis struct #ident #generics (atipicial_contract::types::placeholder::Placeholder) #semi_token
...
fn expand_named_field(fields: &syn::FieldsNamed) -> TokenStream {
    ...
    for (index, field) in fields.named.iter().enumerate() {
        ...
        if let Some(get) = find_attr(field, "get") {
            ...
            expanded.extend(quote::quote! {
                #vis fn #name(&self) -> #ty {
                    atipicial_contract::types::structs::internal_struct_get::<#index, #ty>(self.placeholder)
                }
            });
        } else {
            // getter in default
        ...
        if let Some(set) = find_attr(field, "set") {
            ...
            expanded.extend(quote::quote! {
                #vis fn #name(&mut self, value: #ty) {
                    atipicial_contract::types::structs::internal_struct_set::<#index, #ty>(self.placeholder, value);
                }
            });
        } // no set in default
    }
...
fn expand_unamed_field(fields: &syn::FieldsUnnamed) -> TokenStream {
    ...
    for (index, field) in fields.unnamed.iter().enumerate() {
        ...
        if let Some(get) = find_attr(field, "get") {
            ...
            expanded.extend(quote::quote! {
                #vis fn #name(&self) -> #ty {
                    atipicial_contract::types::structs::internal_struct_get::<#index, #ty>(self.0)
                }
            });
        } else {
            // getter in default
        ...
        if let Some(set) = find_attr(field, "set") {
            ...
            expanded.extend(quote::quote! {
                #vis fn #name(&mut self, value: #ty) {
                    atipicial_contract::types::structs::internal_struct_set::<#index, #ty>(self.0, value);
                }
            });
        } // no set in default
    }
