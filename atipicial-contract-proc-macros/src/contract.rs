...
const AEP11_TOKEN: &str = "Aep11Token";
const AEP17_TOKEN: &str = "Aep17Token";
...
fn expand_impl_item(item: &syn::ItemImpl) -> TokenStream {
    ...
    let mut methods: TokenStream = item
        ...
        .map(|method| {
            ...
            // If the method is marked as safe, add a comment that can be parsed by the WASM to AEF converter
    ...
    if let Some((None, path, _for)) = &item.trait_ {
        // check path is Aep17Token or not
        if path.segments.last().map(|x| x.ident == AEP17_TOKEN).unwrap_or(false) {
            methods.extend(expand_aep17_methods(item));
        }

        if path.segments.last().map(|x| x.ident == AEP11_TOKEN).unwrap_or(false) {
            methods.extend(expand_aep11_methods(item));
        }
    }
...
fn expand_aep17_methods(item: &syn::ItemImpl) -> TokenStream {
    let self_type = item.self_ty.as_ref();
    ...
    if !has_method(item, "transfer") {
        methods.extend(quote::quote! {
            #[no_mangle]
            pub fn transfer(
                from: atipicial_contract::types::H160,
                to: atipicial_contract::types::H160,
                amount: atipicial_contract::types::Int256,
                data: atipicial_contract::types::Array<atipicial_contract::types::Any>,
            ) -> bool {
...
fn expand_aep11_methods(item: &syn::ItemImpl) -> TokenStream {
    let self_type = item.self_ty.as_ref();
    let mut methods: TokenStream = quote::quote! {};
