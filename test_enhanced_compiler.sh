#!/bin/bash
echo "🔧 Testing Enhanced Atipicial WASM→AEF Compiler"

# Test that compiler binary works
./target/release/atipicial-compiler --help > /dev/null && {
...
echo "✅ Memory model translation (WASM linear memory → Atipicial storage)"
...
echo "   AFTER:  Complete WASM→AEF instruction translation pipeline"
...
./target/release/atipicial-compiler compile-all --debug 2>/dev/null | head -10 || {
...
echo "   • Memory model: Linear memory → Atipicial storage mapping"
