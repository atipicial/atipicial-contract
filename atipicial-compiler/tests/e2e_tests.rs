use atipicial_compiler::{AtipicialCompiler, manifest::Manifest};
use std::fs;
...
[dependencies]
atipicial-contract = {{ path = "../../../atipicial-contract" }}
...
#[test]
#[ignore] // Requires cargo and atipicial-contract to be built
...
#[test]
#[ignore] // Requires cargo and atipicial-contract to be built
fn test_e2e_aep17_token_contract() {
    let rust_code = r#"
#![no_std]
#![no_main]
...
"#;
    ...
    // Compile Rust to WASM
    let wasm_path = compile_rust_to_wasm(rust_code, "aep17_token")
        .expect("Failed to compile Rust to WASM");
...
#[test]
fn test_aef_size_limits() {
    // AEF files have size limits we should respect
    ...
    // AEF should be within reasonable size limits
    let aef_size = result.aef.size();
    assert!(aef_size < 1024 * 1024); // Less than 1MB
    
    // Script should be within VM limits
    assert!(result.aef.script.len() < 65536); // Less than 64KB
}