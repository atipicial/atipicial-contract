//! Comprehensive Compiler Tests for Atipicial Rust Framework
//! 
//! Tests cover WASM parsing, AEF generation, manifest creation,
...
/// AEF generation and validation tests
mod aef_generation_tests {
    use super::*;

    #[test]
    fn test_aef_structure_validation() {
        let script = vec![0x0B]; // Simple return instruction
        let compiler = "test-compiler".to_string();
        let source = "test-source".to_string();
        
        let aef = Aef3::new(compiler.clone(), source.clone(), script.clone());
        
        // Validate AEF structure
        assert_eq!(aef.magic, Aef3::MAGIC);
        assert_eq!(aef.compiler, compiler);
        assert_eq!(aef.source, source);
        assert_eq!(aef.script, script);
        assert_ne!(aef.checksum, 0); // Should have valid checksum
    }

    #[test]
    fn test_aef_checksum_calculation() {
        let script1 = vec![0x0B];
        let script2 = vec![0x0C];
        
        let aef1 = Aef3::new("compiler".to_string(), "source".to_string(), script1);
        let aef2 = Aef3::new("compiler".to_string(), "source".to_string(), script2);
        
        // Different scripts should have different checksums
        assert_ne!(aef1.checksum, aef2.checksum);
    }

    #[test]
    fn test_aef_serialization() {
        let script = vec![0x0B, 0x20, 0x41, 0x9C]; // Sample script
        let aef = Aef3::new("atipicial-compiler".to_string(), "test".to_string(), script);
        
        let temp_dir = TempDir::new().unwrap();
        let aef_path = temp_dir.path().join("test.aef");
        
        // Test AEF file writing
        let write_result = aef.write_to_file(&aef_path);
        assert!(write_result.is_ok(), "AEF writing should succeed");
        
        // Test AEF file reading
        let read_result = Aef3::read_from_file(&aef_path);
        assert!(read_result.is_ok(), "AEF reading should succeed");
        
        let read_aef = read_result.unwrap();
        assert_eq!(read_aef.magic, aef.magic);
        assert_eq!(read_aef.script, aef.script);
        assert_eq!(read_aef.checksum, aef.checksum);
    }

    #[test]
    fn test_aef_file_validation() {
        let temp_dir = TempDir::new().unwrap();
        let invalid_aef_path = temp_dir.path().join("invalid.aef");
        
        // Create invalid AEF file
        fs::write(&invalid_aef_path, b"INVALID_AEF_DATA").unwrap();
        
        let read_result = Aef3::read_from_file(&invalid_aef_path);
        assert!(read_result.is_err(), "Invalid AEF should be rejected");
    }
}
...
mod manifest_tests {
    ...
    #[test]
    fn test_aep_standard_detection() {
        let mut manifest = ContractManifest::new();
        
        // Simulate AEP-17 method detection
        let aep17_methods = vec!["symbol", "decimals", "totalSupply", "balanceOf", "transfer"];
        
        for method_name in aep17_methods {
        ...
        assert!(has_symbol && has_transfer && has_balance_of, "Should detect AEP-17 pattern");
    }
...
mod compilation_pipeline_tests {
    ...
    fn test_end_to_end_compilation() {
        ...
        assert!(result.aef_path.exists(), "AEF file should be created");
...
/// AEF verification tests
mod aef_verification_tests {
    use super::*;

    #[test]
    fn test_aef_checksum_verification() {
        let script = vec![0x0B, 0x20, 0x41, 0x9C];
        let aef = Aef3::new("test".to_string(), "test".to_string(), script.clone());
        
        // Test checksum calculation
        let calculated_checksum = aef.calculate_full_checksum().unwrap();
        assert_eq!(aef.checksum, calculated_checksum);
        
        // Test checksum validation
        assert!(aef.validate_checksum(), "Checksum should be valid");
    }

    #[test]
    fn test_aef_script_validation() {
        // Test valid script
        let valid_script = vec![0x0B]; // RETURN
        let valid_aef = Aef3::new("test".to_string(), "test".to_string(), valid_script);
        assert!(!valid_aef.script.is_empty(), "Script should not be empty");
        
        // Test empty script
        let empty_script = vec![];
        let empty_aef = Aef3::new("test".to_string(), "test".to_string(), empty_script);
        assert!(empty_aef.script.is_empty(), "Empty script should be detected");
    }

    #[test]
    fn test_aef_metadata_validation() {
        let script = vec![0x0B];
        
        // Test compiler field truncation
        let long_compiler = "a".repeat(100);
        let aef = Aef3::new(long_compiler, "source".to_string(), script.clone());
        assert!(aef.compiler.len() <= 64, "Compiler field should be truncated to 64 chars");
        
        // Test source field truncation
        let long_source = "b".repeat(300);
        let aef = Aef3::new("compiler".to_string(), long_source, script);
        assert!(aef.source.len() <= 256, "Source field should be truncated to 256 chars");
    }
}
...
mod error_handling_tests {
    ...
    fn test_output_directory_creation() {
        ...
        let compiler = AtipicialCompiler::new().with_output_dir(output_dir.clone());
...
mod cli_tests {
    ...
    fn test_cli_argument_parsing() {
        ...
        let args = vec!["atipicial-compiler", "compile", "input.wasm"];
...
mod performance_tests {
    ...
    fn test_large_script_compilation() {
        ...
        let aef = Aef3::new("perf-test".to_string(), "large".to_string(), large_script);
        assert!(aef.script.len() > 1000, "Large script should be preserved");
        assert_ne!(aef.checksum, 0, "Large script should have valid checksum");
    }
    ...
    fn test_compilation_speed() {
        ...
        let compiler = AtipicialCompiler::new().with_output_dir(output_dir);
