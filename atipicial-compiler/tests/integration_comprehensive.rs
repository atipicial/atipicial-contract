//! Comprehensive Integration Tests for Atipicial Compiler
//! 
//! End-to-end testing of the complete compilation pipeline from
//! WASM input to deployable AEF output with manifest generation.
...
mod pipeline_integration_tests {
    ...
    fn test_hello_world_compilation_pipeline() {
        ...
        assert!(compilation_result.aef_path.exists(), "AEF file should be created");
        assert!(compilation_result.manifest_path.exists(), "Manifest file should be created");
        
        // Validate AEF content
        let aef_content = fs::read(&compilation_result.aef_path).unwrap();
        assert!(aef_content.len() > 320, "AEF should have header + script"); // Standard AEF header is ~320 bytes
        assert_eq!(&aef_content[0..4], b"AEF3", "Should have AEF3 magic");
    ...
    #[test]
    fn test_aep17_token_compilation_pipeline() {
        // Test compilation of a more complex AEP-17 token contract
        let temp_dir = TempDir::new().unwrap();
        
        let aep17_wasm = create_aep17_wasm();
        let wasm_path = temp_dir.path().join("aep17_token.wasm");
        fs::write(&wasm_path, aep17_wasm).unwrap();
        
        let output_dir = temp_dir.path().join("aep17_output");
        
        let compiler = AtipicialCompiler::new()
            .with_source("aep17_test".to_string())
            .with_output_dir(output_dir);
        
        let result = compiler.compile(&wasm_path);
        assert!(result.is_ok(), "AEP-17 compilation should succeed");
        
        let compilation_result = result.unwrap();
        
        // Validate manifest has AEP-17 methods
        let manifest_content = fs::read_to_string(&compilation_result.manifest_path).unwrap();
        let manifest: serde_json::Value = serde_json::from_str(&manifest_content).unwrap();
        
        let methods = manifest["abi"]["methods"].as_array().unwrap();
        
        // Check for AEP-17 required methods
        let method_names: Vec<String> = methods.iter()
            .map(|m| m["name"].as_str().unwrap().to_string())
            .collect();
        
        let aep17_methods = vec!["symbol", "decimals", "totalSupply", "balanceOf", "transfer"];
        for required_method in aep17_methods {
    ...
    fn test_complex_defi_compilation() {
        ...
        let aef_content = fs::read(&compilation_result.aef_path).unwrap();
        assert!(aef_content.len() > 500, "Complex contract should have substantial script");
        ...
        assert_eq!(manifest["extra"]["compiler"], "atipicial-compiler");
    }
    ...
    fn create_aep17_wasm() -> Vec<u8> {
        // Create WASM representing AEP-17 token contract
        ...
        // Export section: export AEP-17 methods
        wasm.extend_from_slice(&[
            0x07, 0x2E, 0x06,
    ...
    fn test_batch_compilation() {
        ...
        for (name, result) in compilation_results {
            assert!(result.aef_path.exists(), "{} AEF should exist", name);
...
mod deployment_simulation_tests {
    ...
    fn test_mainnet_deployment_simulation() {
        ...
        assert!(aef_content.len() > 320, "Production AEF should have content");
        assert!(aef_content.len() < 1024 * 1024, "AEF should be under 1MB limit");
...
mod tooling_integration_tests {
    ...
    fn test_verify_command_integration() {
        ...
        let verify_result = compiler.verify_aef(&compile_result.aef_path);
        assert!(verify_result.is_ok(), "AEF verification should succeed");
