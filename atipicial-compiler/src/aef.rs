...
/// ATC Executable Format version 3
...
impl Aef3 {
    /// AEF3 magic number
    pub const MAGIC: u32 = 0x3346454E; // "AEF3" in little-endian

    /// Create a new AEF3 structure
    pub fn new(compiler: String, source: String, script: Vec<u8>) -> Self {
        ...
        // Calculate checksum over the entire AEF (minus checksum field)
        aef.checksum = aef.calculate_full_checksum().unwrap_or(0);
        aef
    }
    ...
    /// Calculate checksum for the entire AEF file (excluding checksum itself)
    ...
    /// Deserialize AEF from bytes
    pub fn from_bytes(bytes: &[u8]) -> Result<Self> {
        if bytes.len() < 4 {
            anyhow::bail!("Invalid AEF: too short");
        }
        ...
        if magic != Self::MAGIC {
            anyhow::bail!("Invalid AEF magic: 0x{:08X}", magic);
        }
        cursor += 4;
    ...
    /// Get the size of the AEF in bytes
...
mod tests {
    ...
    fn test_aef_creation() {
        ...
        assert_eq!(aef.compiler, "test-compiler");
        assert_eq!(aef.source, "test.rs");
        assert_eq!(aef.script, script);
        assert!(aef.verify_checksum());
    }

    #[test]
    fn test_aef_serialization() {
        let script = vec![0x10, 0x20, 0x30];
        let mut aef = Aef3::new(
            "compiler".to_string(),
            "source".to_string(),
            script,
        );
        ...
        // Add a method token
        aef.add_token(MethodToken::new(
            vec![0; 20],
            "transfer".to_string(),
            3,
        ));
        
        let bytes = aef.to_bytes().unwrap();
        let deserialized = Aef3::from_bytes(&bytes).unwrap();
        
        assert_eq!(aef.magic, deserialized.magic);
        assert_eq!(aef.compiler, deserialized.compiler);
        assert_eq!(aef.source, deserialized.source);
        assert_eq!(aef.script, deserialized.script);
        assert_eq!(aef.tokens.len(), deserialized.tokens.len());
    }
    ...
    fn test_string_truncation() {
        ...
        assert!(aef.compiler.len() <= 64);
        assert!(aef.source.len() <= 256);
    }
}