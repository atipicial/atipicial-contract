...
use crate::aef::{Aef3, MethodToken};
...
/// WASM to ATC VM bytecode translator
...
impl WasmTranslator {
    ...
    pub fn translate(&mut self, module: &WasmModule, manifest: &Manifest, source: &str) -> Result<Aef3> {
        ...
        // Create AEF with optimized script
        let mut aef = Aef3::new(
            "atipicial-compiler".to_string(),
            source.to_string(),
            optimized_script,
        );
        ...
        for import in &module.imports {
            if import.module == "atipicial" || import.module == "System" {
                // Add syscall token
    ...
    fn emit_entry_point(&mut self, manifest: &Manifest) -> Result<()> {
        // Generate proper ATC contract with method dispatcher
        ...
        if manifest.abi.methods.is_empty() {
            ...
            // Push initial value
            self.emit_push_string("Hello, Atipicial!")?;
            // Push key
        ...
        for method in &manifest.abi.methods {
            ...
            match method.name.as_str() {
                "hello" | "sayHello" => {
                    self.emit_push_string("Hello from Atipicial!")?;
                },
                "initialize" => {
    ...
    fn emit_enhanced_entry_point(&mut self, module: &WasmModule, manifest: &Manifest) -> Result<()> {
        ...
        for method in &manifest.abi.methods {
            ...
            if !found_export {
                ...
                match method.name.as_str() {
                    "hello" | "sayHello" => {
                        self.emit_push_string("Hello from Enhanced Atipicial!")?;
                    },
                    _ => {
                        self.emit_opcode(OpCode::PushTrue)?;
                    }
    ...
    fn create_syscall_token(&self, name: &str) -> Result<Option<MethodToken>> {
        ...
        if let Some(_id) = syscall_id {
            // In ATC, syscalls don't use method tokens
