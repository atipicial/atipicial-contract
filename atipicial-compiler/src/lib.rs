...
/// Main compiler structure for converting WASM to AEF
...
impl AtipicialCompiler {
    /// Create a new compiler instance
    pub fn new() -> Self {
        Self {
            debug: false,
            source: "atipicial-compiler".to_string(),
    ...
    pub fn compile(&self, wasm_path: &Path) -> Result<CompilationResult> {
        ...
        let aef = translator.translate(&module, &manifest, &self.source)?;
        ...
        let aef_path = self.output_dir.join(format!("{}.aef", base_name));
...
pub struct CompilationResult {
    /// Path to generated AEF file
    pub aef_path: PathBuf,
    /// Path to generated manifest file
    pub manifest_path: PathBuf,
    /// The compiled AEF structure
    pub aef: Aef3,
    /// The manifest
...
impl Default for AtipicialCompiler {
    fn default() -> Self {
        Self::new()
    }
}
