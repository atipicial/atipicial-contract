        ...
        for entry in entries.flatten() {
            ...
            if path.is_dir() {
                ...
                if build_wasm_for_example(&path, example_name).is_ok() {
                    // Compile to AEF if WASM exists
                    let wasm_path = path
                        .join("target/wasm32-unknown-unknown/release")
                        .join(format!("{}.wasm", example_name.replace('-', "_")));
                    
                    if wasm_path.exists() {
                        compile_to_aef(&wasm_path, &build_dir, example_name);
                    }
                }
            }
        }
...
fn compile_to_aef(wasm_path: &Path, build_dir: &Path, example_name: &str) {
    // Check if atipicial-compiler is built
    let compiler_path = PathBuf::from("target/release/atipicial-compiler");
    if !compiler_path.exists() {
        // Try debug build
        let debug_compiler = PathBuf::from("target/debug/atipicial-compiler");
        if !debug_compiler.exists() {
            println!("Atipicial compiler not found, skipping AEF compilation for {}", example_name);
            return;
        }
    }
    
    // Run the atipicial-compiler
    let output = Command::new(if compiler_path.exists() { &compiler_path } else { &PathBuf::from("target/debug/atipicial-compiler") })
        .args(&[
            "compile",
    ...
    match output {
        Ok(output) if output.status.success() => {
            println!("Successfully compiled {} to AEF", example_name);
        }
        Ok(output) => {
            let stderr = String::from_utf8_lossy(&output.stderr);
            eprintln!("Warning: Failed to compile {} to AEF: {}", example_name, stderr);
        }
        Err(e) => {
            eprintln!("Warning: Failed to run atipicial-compiler for {}: {}", example_name, e);
        }
    }
}