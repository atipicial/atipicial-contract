...
class DeFiTester:
    def __init__(self, network="defi-testnet.atipicial-express"):
    ...
    def test_uniswap_swap(self):
        ...
        cmd = f'''atipicial-express contract invoke {self.contracts["USDT"]} approve \\
        ...
        # Execute swap
        cmd = f'''atipicial-express contract invoke uniswap-v2-amm swap \\
                --arg "{{\\"type\\":\\"Integer\\",\\"value\\":\\"100000000000\\"}}" \\
    ...
    def test_compound_lending(self):
        ...
        # Supply USDT
        cmd = f'''atipicial-express contract invoke compound-lending supply \\
                --arg "{{\\"type\\":\\"Integer\\",\\"value\\":\\"500000000000\\"}}" \\
                bob -i {self.network}'''
        self.run_command(cmd)
        ...
        cmd = f'''atipicial-express contract invoke compound-lending borrow \\
    ...
    def test_aave_flashloan(self):
        ...
        cmd = f'''atipicial-express contract invoke aave-flashloan flash_loan \\
                --arg "{{\\"type\\":\\"Hash160\\",\\"value\\":\\"{self.contracts["USDT"]}\\"}}" \\
    ...
    def test_liquidity_provision(self):
        ...
        for token in ["USDT", "WETH"]:
            cmd = f'''atipicial-express contract invoke {self.contracts[token]} approve \\
                    --arg "{{\\"type\\":\\"Hash160\\",\\"value\\":\\"uniswap-v2-amm\\"}}" \\
                    --arg "{{\\"type\\":\\"Integer\\",\\"value\\":\\"1000000000000\\"}}" \\
                    alice -i {self.network}'''
            self.run_command(cmd)
        ...
        # Add liquidity
        cmd = f'''atipicial-express contract invoke uniswap-v2-amm add_liquidity \\
                --arg "{{\\"type\\":\\"Integer\\",\\"value\\":\\"100000000000\\"}}" \\
    ...
    def test_liquidation(self):
        ...
        cmd = f'''atipicial-express contract invoke compound-lending liquidate \\
    ...
    def check_balance(self, user, token):
        ...
        cmd = f'''atipicial-express contract invoke {self.contracts[token]} balance_of \\
