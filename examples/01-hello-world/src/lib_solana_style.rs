...
//!
//! A Atipicial smart contract using Solana-style syntax demonstrating:
...
// Declare the program ID (Atipicial contract address)
declare_id!("AtipicialHelloWorldProgram123456789");
...
pub mod hello_world {
    ...
    pub fn get_info(ctx: Context<GetInfo>) -> Result<ContractInfo> {
        ...
        Ok(ContractInfo {
            ...
            author: "Atipicial Rust Framework".to_string(),
    ...
    pub fn reset(ctx: Context<Reset>) -> Result<()> {
        ...
        // Reset to defaults
        state.greeting = "Hello, Atipicial World!".to_string();
...
macro_rules! emit {
    ($event:expr) => {
        {
            atipicial_contract::emit_event($event)
        }
    };
}
...
macro_rules! msg {
    ($($arg:tt)*) => {
        {
            atipicial_contract::log_message(&format!($($arg)*))
        }
    };
}
