...
echo "=== Comprehensive Atipicial Compilation Fixes ==="

# Examples with duplicate error code issues (need conversion from Solana to Atipicial style)
...
for example in "${solana_style_examples[@]}"; do
    ...
    if [ -f "$lib_file" ]; then
        ...
        if echo "$error_output" | grep -q "defined multiple times"; then
            echo "  ⚠️  Has duplicate error definitions - needs Solana->Atipicial conversion"
...
if [ $success_count -lt $total_examples ]; then
    ...
    echo "  - Solana-style examples need complete conversion to Atipicial syntax"
