...
for CONTRACT in "${CONTRACTS[@]}"; do
    AEF_FILE="target/wasm32-unknown-unknown/release/${CONTRACT}.aef"
    ...
    if [ -f "$AEF_FILE" ] && [ -f "$MANIFEST_FILE" ]; then
        # Run verification
        if python3 scripts/verify_aef_manifest.py "$AEF_FILE" "$MANIFEST_FILE" > /tmp/verify_${CONTRACT}.log 2>&1; then
            echo -e "${GREEN}✓ $CONTRACT is valid and deployable!${NC}"
            VALID=$((VALID + 1))
            
            # Show key details
            echo -e "  AEF size: $(stat -c%s "$AEF_FILE" 2>/dev/null || stat -f%z "$AEF_FILE") bytes"
        ...
        [ ! -f "$AEF_FILE" ] && echo -e "  ${RED}Missing: $AEF_FILE${NC}"
...
# List all AEF files
echo -e "\n${CYAN}AEF Files:${NC}"
ls -lh target/wasm32-unknown-unknown/release/*.aef 2>/dev/null | awk '{print "  " $9 " (" $5 ")"}'

# List all manifest files
echo -e "\n${CYAN}Manifest Files:${NC}"
ls -lh target/wasm32-unknown-unknown/release/*.manifest.json 2>/dev/null | awk '{print "  " $9 " (" $5 ")"}'

echo -e "\n${GREEN}All DeFi contracts have been compiled to AEF format!${NC}"