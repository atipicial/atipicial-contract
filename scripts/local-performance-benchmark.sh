...
echo -e "${BLUE}║           Atipicial Framework Performance Validation             ║${NC}"
echo -e "${BLUE}╚════════════════════════════════════════════════════════════════╝${NC}"
...
cargo build --workspace --release >/dev/null 2>&1 || {
    ...
    cargo build -p atipicial-contract -p atipicial-compiler -p atipicial-contract-proc-macros --release >/dev/null 2>&1
}
END_TIME=$(date +%s.%N)
...
components=("atipicial-contract" "atipicial-compiler" "atipicial-contract-proc-macros")
...
if [ $PERF_SCORE -ge 80 ]; then
    ...
    echo -e "\n⚠️ ${YELLOW}Framework performance may beaefit from optimization${NC}"
fi
