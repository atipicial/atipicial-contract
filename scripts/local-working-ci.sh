...
echo -e "${BLUE}║              Atipicial Framework Development Workflow            ║${NC}"
echo -e "${BLUE}╚════════════════════════════════════════════════════════════════╝${NC}"
...
echo -n "  Building atipicial-contract... "
if cargo build -p atipicial-contract --release >/dev/null 2>&1; then
    echo -e "${GREEN}✅${NC}"
else
    echo -e "${RED}❌${NC}"
    echo -e "${RED}Error details:${NC}"
    cargo build -p atipicial-contract --release
    exit 1
fi

echo -n "  Building atipicial-compiler... "
...
echo -n "  Building atipicial-contract-proc-macros... "
if cargo build -p atipicial-contract-proc-macros --release >/dev/null 2>&1; then
    echo -e "${GREEN}✅${NC}"
else
    echo -e "${RED}❌${NC}"
    echo -e "${RED}Error details:${NC}"
    cargo build -p atipicial-contract-proc-macros --release
    exit 1
fi

echo -e "${GREEN}✅ All core components built successfully${NC}"

echo -e "${BLUE}Testing atipicial-contract library...${NC}"
if cargo test -p atipicial-contract --lib >/dev/null 2>&1; then
    ...
    cargo test -p atipicial-contract --lib
    exit 1
fi
