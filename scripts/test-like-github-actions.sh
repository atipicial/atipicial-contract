...
echo -n "  Building atipicial-contract with nightly... "
if cargo build -p atipicial-contract --release >/dev/null 2>&1; then
    echo -e "${GREEN}✅${NC}"
else
    echo -e "${RED}❌${NC}"
    echo -e "${RED}Compilation errors with nightly:${NC}"
    cargo build -p atipicial-contract --release 2>&1 | head -20
    echo ""
    echo -e "${YELLOW}This explains why GitHub Actions fails but local stable succeeds!${NC}"
fi

echo -n "  Building atipicial-compiler with nightly... "
if cargo build -p atipicial-compiler --release >/dev/null 2>&1; then
    echo -e "${GREEN}✅${NC}"
else
    echo -e "${RED}❌${NC}"
    cargo build -p atipicial-compiler --release 2>&1 | head -10
fi

echo -n "  Building atipicial-contract-proc-macros with nightly... "
if cargo build -p atipicial-contract-proc-macros --release >/dev/null 2>&1; then
    echo -e "${GREEN}✅${NC}"
else
    echo -e "${RED}❌${NC}"
    cargo build -p atipicial-contract-proc-macros --release 2>&1 | head -10
fi
...
if env RUSTFLAGS="-Ctarget-feature=+multivalue -Clink-arg=--initial-memory=2097152" cargo check --target wasm32-unknown-unknown -p atipicial-contract >/dev/null 2>&1; then
    echo -e "${GREEN}✅${NC}"
else
    echo -e "${RED}❌${NC}"
    echo -e "${RED}WASM compilation errors:${NC}"
    env RUSTFLAGS="-Ctarget-feature=+multivalue -Clink-arg=--initial-memory=2097152" cargo check --target wasm32-unknown-unknown -p atipicial-contract 2>&1 | head -10
fi
