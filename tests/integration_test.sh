...
# Atipicial Smart Contract Integration Test Suite
# Tests compilation, AEF generation, and deployment for all examples
...
# Function to test AEF generation
test_aef_generation() {
    local example_dir=$1
    local example_name=$(basename "$example_dir")
    
    print_test "Testing AEF generation for $example_name"
    ...
    # Test AEF compilation
    mkdir -p "$example_dir/build"
    
    if cargo run -p atipicial-compiler -- compile "$wasm_file" --output "$example_dir/build" > /dev/null 2>&1; then
        print_info "✅ AEF generation passed for $example_name"
        PASSED_TESTS=$((PASSED_TESTS + 1))
        
        # Check if AEF file exists
        if [ -f "$example_dir/build/"*.aef ]; then
            print_info "✅ AEF file created for $example_name"
        else
            print_error "❌ AEF file not found for $example_name"
            FAILED_TESTS=$((FAILED_TESTS + 1))
        fi
    else
        print_error "❌ AEF generation failed for $example_name"
        FAILED_TESTS=$((FAILED_TESTS + 1))
        return 1
    fi
...
main() {
    print_info "=========================================="
    print_info "Atipicial Smart Contract Integration Test Suite"
    print_info "=========================================="
    ...
    run_test "atipicial-contract library" "cargo check -p atipicial-contract"
    run_test "atipicial-contract-proc-macros" "cargo check -p atipicial-contract-proc-macros"
    run_test "atipicial-compiler" "cargo check -p atipicial-compiler"
    ...
    # Test AEF generation for a subset of examples
    print_test "Testing AEF generation..."
    
    for example_dir in examples/01-hello-world examples/03-counter examples/04-aep17-token; do
        if [ -d "$example_dir" ]; then
            test_aef_generation "$example_dir"
        fi
    done
