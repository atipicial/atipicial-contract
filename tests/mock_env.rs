...
//! Mock Atipicial environment for testing purposes.
//! This module provides a simulated Atipicial blockchain environment
//! that can be used for unit testing Atipicial smart contracts.
...
/// Mock implementation of the Atipicial blockchain environment
pub struct MockAtipicialEnvironment {
...
pub trait WithMockEnv {
    ...
    fn new_with_env(env: &mut MockAtipicialEnvironment) -> Self;
}
...
pub mod runtime {
    ...
    pub fn check_witness_with_env(env: &MockAtipicialEnvironment, account: H160) -> bool {
        env.has_witness(account)
    }

    /// Emit a notification event
    pub fn notify_with_env(env: &mut MockAtipicialEnvironment, name: ByteString, data: Any) {
        env.add_notification(name, data);
    }

    /// Get the current timestamp
    pub fn get_timestamp_with_env(env: &MockAtipicialEnvironment) -> u64 {
        env.get_timestamp()
    }

    /// Get the remaining gas
    pub fn gas_left_with_env(env: &MockAtipicialEnvironment) -> u64 {
        env.get_gas_left()
    }

    /// Get the executing script hash
    pub fn executing_script_hash_with_env(env: &MockAtipicialEnvironment) -> H160 {
        env.executing_script_hash()
    }

    /// Get the calling script hash
    pub fn calling_script_hash_with_env(env: &MockAtipicialEnvironment) -> H160 {
        env.calling_script_hash()
    }

    /// Get the entry script hash
    pub fn entry_script_hash_with_env(env: &MockAtipicialEnvironment) -> H160 {
...
pub mod storage {
    ...
    impl<'a> MockStorageMap<'a> {
        ...
        pub fn new(env: &'a mut MockAtipicialEnvironment) -> Self {
            Self { env }
        }
