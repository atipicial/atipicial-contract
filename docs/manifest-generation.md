...
This document describes how manifest generation works in the **Atipicial Rust Smart Contract Framework** using the atipicial-wasm compiler.

## Overview

Atipicial smart contracts require a **manifest file** that defines the contract's metadata, permissions, and Application Binary Interface (ABI). This manifest is essential for the Atipicial VM to properly interact with the contract and for external applications to understand the contract's interface.

The framework **automatically generates** manifests using the **atipicial-wasm compiler** from:
...
```bash
# Generate manifest using atipicial-wasm compiler
make manifest

# Or build everything (WASM → AEF + Manifest)
make
```

### Atipicial-WASM Compiler Commands

The manifest generation uses these atipicial-wasm commands:

```bash
# Generate manifest with source code integration
atipicial-wasm translate --input contract.wasm --manifest contract.manifest.json --source-code src/lib.rs

# Generate manifest from WASM only
atipicial-wasm translate --input contract.wasm --manifest contract.manifest.json
```
...
The atipicial-wasm compiler analyzes the compiled WASM file to extract:
...
The compiler generates a complete Atipicial manifest with:
...
- **Standards detection** - AEP-17, AEP-11 compliance
...
Here's an example of a generated manifest for a AEP-17 token:
...
```json
{
  ...
  "trusts": [],
  "extra": {
    "Author": "Atipicial Rust Framework",
    "Description": "AEP-17 fungible token implementation",
    "Version": "1.0.0"
  }
}
```
...
- Check that atipicial-wasm compiler is available
- Verify source file path is correct

**Missing method documentation:**
- Add doc comments to Rust methods
- Use `--source-code` parameter with atipicial-wasm
...
- **[Understanding Atipicial Manifests](understanding-atipicial-manifests.md)** - Manifest structure details
