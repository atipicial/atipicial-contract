...
This document describes the implementation of Atipicial syscall support in the atipicial-contract-rs framework.

## Overview

The atipicial-contract-rs framework allows Rust smart contracts to interact with the Atipicial blockchain through syscalls. These syscalls are defined in the Atipicial protocol and provide access to blockchain features such as storage, runtime information, and other blockchain services.

## Architecture

The syscall implementation consists of the following components:

1. **Rust Interface**: Defined in `atipicial-contract/src/env/syscall.rs`
2. **Runtime Wrappers**: Helper functions in `atipicial-contract/src/runtime/mod.rs` that provide a more idiomatic Rust API
3. **Syscall Mappings**: Defined in `atipicial-wasm/rosetta/builtin/syscall.go`, mapping Rust function names to Atipicial syscall names
4. **Translation Logic**: Implemented in `atipicial-wasm/rosetta/translates/syscall.go`, which converts WASM imports to AEF syscall instructions
...
3. During compilation to WASM, these calls become WASM imports with the "atipicial.syscall" module
4. The atipicial-wasm compiler recognizes these imports and maps them to Atipicial VM syscalls
5. In the generated AEF bytecode, these are converted to SYSCALL opcodes with the appropriate system call name
...
```rust
#[link(wasm_import_module = "atipicial.syscall")]
extern "C" {
...
In `rosetta/builtin/syscall.go`, each Rust syscall is mapped to its Atipicial VM counterpart:

```go
syscalls = map[string]Syscall{
    "system_runtime_trigger": {"System.Runtime.Trigger", []atipicial.ParamType{}, atipicial.ParamInteger},
    "system_runtime_platform": {"System.Runtime.Platform", []atipicial.ParamType{}, atipicial.ParamString},
    // ... other mappings
}
```
...
The current implementation supports all standard Atipicial syscalls, including:
...
1. Enable verbose logging: `ATC_WASM_DEBUG=1 ATC_WASM_VERBOSE=1`
2. Check for "Processing syscall import" messages in the logs
3. Verify the syscall name in the generated AEF assembly
