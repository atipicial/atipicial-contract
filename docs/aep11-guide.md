...
This guide provides a comprehensive overview of implementing the AEP-11 Non-Fungible Token (NFT) standard using the atipicial-contract-rs framework.

## Introduction to AEP-11

The AEP-11 standard defines the interface for non-fungible tokens (NFTs) on the Atipicial blockchain. AEP-11 tokens can be:
...
## AEP-11 Interface Requirements

### Core Methods

Every AEP-11 implementation must support these core methods:
...
## Implementing AEP-11 in Rust

Below is a step-by-step guide to implementing AEP-11 tokens using atipicial-contract-rs.
...
## Testing Your AEP-11 Implementation
...
For integration testing, deploy your contract to a Atipicial test network and interact with it using:

```
atipicial-express deploy token.aef
atipicial-express invoke <contract-hash> mint [<params>]
atipicial-express invoke <contract-hash> transfer [<params>]
```
...
pub fn royalty_info(&self, token_id: ByteString) -> Map<H160, Int256> {
    // Return map of beaeficiary addresses to percentage (basis points)
}
```
...
Implementing the AEP-11 standard in Rust with atipicial-contract-rs provides a powerful way to create NFTs on the Atipicial blockchain. This guide covered the basic implementation details, but you can extend it with additional features to suit your specific use case.
