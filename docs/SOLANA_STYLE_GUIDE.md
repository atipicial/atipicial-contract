...
This guide explains how to write Atipicial smart contracts using Solana-style syntax with the atipicial-contract-rs framework. This approach brings familiar and developer-friendly patterns inspired by Solana's development model to Atipicial blockchain development, using `#[contract_impl]` as the primary pattern for contract implementation.
...
Methods can be marked with specific attributes for Atipicial integration:
...
Unlike traditional Context patterns, Solana-style methods use direct parameters with proper Atipicial types:
...
pub fn transfer(&mut self, from: H160, to: H160, amount: U256) -> Result<bool> {
    ...
    // Transfer logic with Atipicial native types
...
#[contract_description("A Solana-style Atipicial contract")]
...
Use Atipicial native storage types for persistent state:
...
Implement proper access control using Atipicial's witness checking:
...
### Atipicial Notifications

Use Atipicial's native notification system:
...
### AEP-17 Token Contract

```rust
#[contract_author("AEP-17 Token")]
#[contract_version("1.0.0")]
#[contract_name("MyToken")]
#[contract_description("A AEP-17 compliant token")]
pub struct Aep17Token {
    pub balances: StorageMap<H160, U256>,
    pub total_supply: U256,
    pub decimals: u8,
...
impl Aep17Token {
    ...
    fn _transfer(
        ...
        // Call onAEP17Payment if recipient is a contract
        if Runtime::get_contract(&to).is_some() {
            if let Some(data) = data {
                Runtime::call_contract(&to, "onAEP17Payment", &[from.into_any(), amount.into_any(), data.into_any()]);
            } else {
                Runtime::call_contract(&to, "onAEP17Payment", &[from.into_any(), amount.into_any()]);
            }
        }
...
5. **Type Safety**: Direct Atipicial types without manual conversion
6. **Events**: `notify!` macro for Atipicial notifications

## Best Practices

1. **Always validate inputs** using `require!` macros
2. **Use checked arithmetic** with Atipicial's built-in safe math
...
8. **Follow AEP standards** for interoperability

## Compiler Integration

The atipicial-wasm compiler automatically detects and processes Solana-style contracts:

1. **Automatic Detection**: Identifies `#[contract_impl]` implementations
2. **Manifest Generation**: Creates Atipicial manifests from contract attributes
3. **Method Detection**: Identifies safe vs. state-changing methods
4. **Entry Point Mapping**: Converts contract methods to Atipicial entry points
...
# Compile to Atipicial bytecode (manifest auto-generated)
atipicial-wasm translate target/wasm32-unknown-unknown/release/my_contract.wasm

# Deploy to Atipicial network
atipicial-cli contract deploy build/my_contract.aef build/my_contract.manifest.json
```
...
See `atipicial-contract/tests/integration_tests.rs` for comprehensive test examples using the Solana-style patterns.
...
- Documentation: [Atipicial Developer Portal](https://developers.atipicial.com/)
