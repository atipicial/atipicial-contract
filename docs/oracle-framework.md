...
This document outlines the design, implementation, and usage patterns for the Oracle framework in the **Atipicial Rust Smart Contract Framework**.

## Overview

The Oracle framework enables Atipicial smart contracts written in Rust to access external data that is not available on the blockchain. This is crucial for many real-world applications that need to interact with external APIs, price feeds, or other off-chain data sources.
...
3. **Request Serialization**: Converts request parameters to format compatible with Atipicial Oracle nodes
...
Oracle requests are made using the Atipicial native oracle service. When a contract makes an oracle request:
...
1. The Atipicial VM executes the contract's callback method
...
# Build and test
make                    # Build WASM → AEF + Manifest
make test              # Run unit tests
```

### Deployment Process

1. **Build Contract** - Generate AEF and manifest files
2. **Deploy to Testnet** - Test with Atipicial testnet
...
- **[Atipicial Oracle Documentation](https://docs.atipicial.com/docs/en-us/reference/oracle.html)** - Official oracle service docs
- **[Atipicial Developer Guide](https://developers.atipicial.com/)** - Atipicial development resources
