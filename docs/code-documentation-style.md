...
This document outlines the recommended style for documenting code in the atipicial-contract-rs framework. Following these guidelines will ensure consistency across the codebase and enable optimal manifest generation.
...
//! other contracts on the ATC blockchain.
```
...
```rust
/// TokenContract implements a standard AEP-17 token with
...
/// - May call "onAEP17Payment" on the receiving contract
/// 
...
```rust
/// The token name as defined in the AEP-17 standard
const TOKEN_NAME: &'static str = "ExampleToken";

/// The token symbol as defined in the AEP-17 standard
...
/// Transfers tokens from one account to another
/// 
/// This method is required by the AEP-17 standard. It verifies
