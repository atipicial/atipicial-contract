...
Welcome to the **Atipicial Rust Smart Contract Framework**! This guide will help you set up your development environment and create your first Atipicial smart contract using Rust with proper AEF and manifest generation.
...
3. **Basic knowledge** of Rust and Atipicial blockchain concepts
4. **Atipicial node** or testnet access for deployment (optional)
...
```bash
git clone https://github.com/Atipicial-Network/atipicial-contract-rs.git
cd atipicial-contract-rs
```
...
```bash
# Build all 13 examples with proper AEF and manifest generation
make build-all

# Or build individual examples
cd examples/01-hello-world
make                    # Build everything (WASM → AEF + Manifest)
```
...
- **📁 `atipicial-contract/`** - Core Rust smart contract library
- **📁 `atipicial-contract-proc-macros/`** - Procedural macros for contract attributes
- **📁 `atipicial-wasm/`** - WASM to AEF compiler
...
Each example uses a **professional Makefile** with proper atipicial-wasm integration:

```bash
# Available build targets
make help           # Show all available targets
make build          # Build Rust to WASM
make aef            # Generate AEF using atipicial-wasm compiler
make manifest       # Generate manifest using atipicial-wasm compiler
...
#[contract_permission("*", "*")]
#[contract_meta("description", "My first Atipicial smart contract")]
#[contract]
...
impl MyContract {
    ...
    pub fn get_greeting(&self) -> ByteString {
        self.greeting.get().unwrap_or_else(|| ByteString::from("Hello, Atipicial!"))
    }
...
```bash
# Build everything (WASM → AEF + Manifest)
make
...
### **Token Standards**
- **`04-aep17-token`** - Fungible token (like ERC-20)
- **`05-aep11-nft`** - Non-fungible token (like ERC-721)
- **`06-aep24-royalty-nft`** - NFT with royalty support
...
## 🪙 **AEP-17 Token Example**

Here's how to build a AEP-17 token using the framework:

```bash
# Use the complete AEP-17 example
...
ls -la build/
# Output:
# 04-aep17-token.aef          - AEF file for deployment
# 04-aep17-token.manifest.json - Contract manifest with ABI
```

The AEP-17 example includes:
...
```bash
# Generate AEF and manifest files
make

# Verify generated files
ls -la build/
# my-contract.aef          - Atipicial executable format
# my-contract.manifest.json - Contract metadata and ABI
```

### 2. Deploy to Atipicial

```bash
# Using Atipicial-CLI
atipicial-cli> deploy build/my-contract.aef build/my-contract.manifest.json

# Or using Atipicial-Express for testing
atipicialxp contract deploy build/my-contract.aef
```
...
- **NFTs**: Explore `examples/05-aep11-nft` for NFT contracts
...
- 🔄 **[Syscall Implementation](atipicial-syscall-implementation.md)** - Low-level operations
...
**🎉 Start building amazing Atipicial smart contracts with Rust!**
